...
import { FilterButton } from '@/app/components/ui/FiltreButtons/FilterButtons';

import { blogFilter, articles } from '../../data/data';
...
export function BlogPage() {
  const { t } = useLanguage();
      ...
      const [filteredArticles, setFilteredArticles] = useState(processedArticles);
      
      function handleFilter(filterType) {
        if (filterType === "all") {
          setFilteredArticles(processedArticles);
        } else {
          const filtered = processedArticles.filter((article) => 
            article.category.includes(filterType),
          )
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <ul className="flex flex-wrap items-center gap-2 mb-8">
          {blogFilter.map((filter) => (
            <li key={filter.id}>
              <FilterButton name={filter.name} onClick={() => handleFilter(filter.filterType)}/>
            </li>
          ))}
        </ul>
